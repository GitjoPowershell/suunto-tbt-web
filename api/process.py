...
WINDOW_M = 50         # look-ahead/behind for bearing computation
MIN_TURN_ANGLE = 15   # degrees — minimum bearing change to record a turn
MIN_SPACING_M = 40    # minimum distance between consecutive TbT waypoints
SNAP_DIST_M = 25      # max distance from route to an OSM junction node
...
def signed_turn(b_in, b_out):
    """Signed angle [-180, 180]: positive = right, negative = left."""
...
def get_osm_junctions(points, padding=0.003):
    """Query Overpass for highway junction nodes in the route's bounding box."""
...
out body;"""
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "suunto-tbt/1.0",
        },
    )
    with urllib.request.urlopen(req, timeout=58) as resp:
        data = json.loads(resp.read())

    # Count occurrences of each node across all ways
    ...
    # A junction is a node shared by 2+ ways (= fork/intersection)
    junctions = []
    for el in data["elements"]:
        if el["type"] == "node" and node_count.get(el["id"], 0) >= 2:
            junctions.append((el["lat"], el["lon"]))

    return junctions


def snap_junctions_to_route(junctions, points):
    """Return sorted list of route-point indices closest to each OSM junction."""
    hits: set[int] = set()
...
def detect_turns_at_junctions(points, junction_indices):
    """Emit TbT waypoints only at OSM-confirmed forks with significant bearing change."""
    n = len(points)
    cum = cumulative_distances(points)
    turns = []
    last_d = -MIN_SPACING_M
    ...
    for i in junction_indices:
        ...
        last_d = d
        wpt_type, name = _classify_turn(turn)
        turns.append((points[i][0], points[i][1], name, wpt_type))

    return turns


def detect_turns_angle_fallback(points):
    """Fallback: pure heading-change detection (used if Overpass is unavailable)."""
    n = len(points)
    cum = cumulative_distances(points)
    turns = []
    last_d = -MIN_SPACING_M
...
def _wpt(lat, lon, name, wpt_type, auto_desc=True):
    desc = "<desc>Auto-generated</desc>" if auto_desc else ""
    return f'<wpt lat="{lat}" lon="{lon}"><name>{name}</name>{desc}<type>{wpt_type}</type></wpt>'


def _rtept(lat, lon, ele):
    e = round(ele) if ele is not None else 0
    return f'  <rtept lat="{lat}" lon="{lon}"><ele>{e}</ele></rtept>'
...
def generate_suunto_gpx(points, turn_waypoints):
    ...
    if turn_waypoints:
        ...
        lines += [_wpt(lat, lon, name, t, auto_desc=False) for lat, lon, name, t in turn_waypoints]
    lines += [
        "<!-- Route points -->",
        "<rte>",
        "  <name>Suunto Route</name>",
        "  <desc>Route created with Suunto Routeplanner</desc>",
        f"  <extensions><suunto-rp:waypointIndices>0,{last_idx}</suunto-rp:waypointIndices></extensions>",
    ]
    lines += [_rtept(lat, lon, ele) for lat, lon, ele in points]
    lines += ["</rte>", "</gpx>"]
    return "\n".join(lines)

# ── Entry point ────────────────────────────────────────────────────────────────

def process_gpx(gpx_text: str) -> dict:
    ...
    if not points:
        raise ValueError("No track or route points found in GPX")

    # Detect TbT turns: OSM junctions first, fallback to angle-based
    try:
        junctions = get_osm_junctions(points)
        junction_indices = snap_junctions_to_route(junctions, points)
        turn_waypoints = detect_turns_at_junctions(points, junction_indices)
        method = "osm"
    except Exception:
        turn_waypoints = detect_turns_angle_fallback(points)
        method = "angle"
